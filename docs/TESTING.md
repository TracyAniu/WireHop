...
Discovery tests must never broadcast: a test that sprayed the real network would pick up genuine peers and stop being deterministic. Both suites bind loopback and use explicit unicast targets. The same rule is why no mDNS responder exists yet — a real one multicasts on the developer's network — so `core::dnssd` covers the schema deterministically and real Bonjour interoperability (`dns-sd -B _wirehop._tcp`) remains a manual check for the shell milestone.
