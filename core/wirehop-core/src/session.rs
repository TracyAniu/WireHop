...
/// Receives a transfer into `download_dir`.
///
/// `accept` stands in for the user's decision, which a GUI would prompt for
/// after showing the session code.
pub fn receive_files(
    ...
    stream
        .write_all(&frame::encode(
            &crypto,
            &message::response_to_canonical_json(accept),
        )?)
        .map_err(Error::Io)?;
    ...
    for (index, meta) in metadata.files.iter().enumerate() {
        let mut partial = PartialFile::create(download_dir, index as u32)?;
        let mut remaining = meta.size;

        while remaining > 0 {
            if pending.is_empty() {
                pending = next_frame(stream, &mut reader, &crypto)?;
            }
    ...
    // Best effort: a failure here must not fail a transfer already committed
    // to disk, and legacy senders ignore the frame entirely.
    if let Ok(ack) = frame::encode(&crypto, &message::ack_to_canonical_json()) {
